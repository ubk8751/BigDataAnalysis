...
class CloneDetector {
    ...
    #expandCloneCandidates(file) {
        ...
        const expandedClones = file.instances.reduce((acc, currentClone) => {
            const cloneExpanded = acc.some(existingClone => existingClone.maybeExpandWith(currentClone));
            if (!cloneExpanded) {
    ...
    matchDetect(file) {
        ...
        for (let f of allFiles) {
            // Find all equal chunks in file and f. Represent each matching pair as a Clone.
            file = this.#filterCloneCandidates(file, f);
    
            // For each Clone with endLine=x, merge it with Clone with endLine-1=x
                ...
                if (currentClone.endLine > 0) {
                    for (let j = 0; j < file.instances.length; j++) {
                        if (i !== j) {
                            ...
                            if (currentClone.endLine - 1 === otherClone.endLine && currentClone.equalsExceptEndLine(otherClone)) {
            ...
            // If the same clone is found in several places, consolidate them into one Clone.
